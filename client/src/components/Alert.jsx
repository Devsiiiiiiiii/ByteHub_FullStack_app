...
const Alert = ({type, message}) => {
    if (type === "success"){
        return (
        <motion.div {...fadeInOut} className="fixed z-50 top-32 right-12 px-4 py-2 rounded-md backdrop-blur-sm bg-green-300 shadow-md flex items-center gap-4">
               <FaCheck className="text-xl text-white-700"/>
               <p className='text-xl text-white-700'>{message}</p>
        </motion.div>
     ) ;
        }
